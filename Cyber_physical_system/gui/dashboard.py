...
class CPSDashboard:
    ...
    def _setup_gui(self) -> None:
        ...
        self.status_bar = tk.Label(
            self.root, text="System Ready", bd=1, relief=tk.SUNKEN, anchor=tk.W
        )
    ...
    def _setup_overview_tab(self) -> None:
        ...
        title_label = tk.Label(
            self.tab_overview,
            text="Cyber-Physical System Overview",
            font=("Arial", 16, "bold"),
        )
        ...
        status_frame = ttk.LabelFrame(
            self.tab_overview, text="System Status", padding=10
        )
        ...
        for i, (label, initial_value) in enumerate(status_items):
            tk.Label(
                status_frame, text=f"{label}:", font=("Arial", 10, "bold")
            ).grid(row=i, column=0, sticky=tk.W, padx=5, pady=2)
            value_label = tk.Label(
                status_frame, text=initial_value, font=("Arial", 10)
            )
        ...
        control_frame = ttk.LabelFrame(
            self.tab_overview, text="System Controls", padding=10
        )
        ...
        btn_reset = ttk.Button(
            control_frame, text="Reset Metrics", command=self._reset_metrics
        )
        btn_reset.pack(side=tk.LEFT, padx=5)

        # Recent activity log
        log_frame = ttk.LabelFrame(
            self.tab_overview, text="Activity Log", padding=10
        )
    ...
    def _setup_detection_tab(self) -> None:
        ...
        control_panel = ttk.LabelFrame(
            self.tab_detection, text="Detection Controls", padding=10
        )
        control_panel.pack(fill=tk.X, padx=10, pady=5)

        # Image selection
        tk.Label(control_panel, text="Image:").grid(
            row=0, column=0, padx=5, pady=5
        )
        self.detection_image_path = tk.StringVar()
        entry_image = ttk.Entry(
            control_panel, textvariable=self.detection_image_path, width=50
        )
        entry_image.grid(row=0, column=1, padx=5, pady=5)

        btn_browse = ttk.Button(
            control_panel, text="Browse", command=self._browse_detection_image
        )
        btn_browse.grid(row=0, column=2, padx=5, pady=5)

        # Confidence threshold
        tk.Label(control_panel, text="Confidence:").grid(
        ...
        scale_conf = ttk.Scale(
            ...
            from_=0.1,
            to=1.0,
            variable=self.detection_confidence,
            orient=tk.HORIZONTAL,
            length=200,
        )
        scale_conf.grid(row=1, column=1, padx=5, pady=5, sticky=tk.W)

        tk.Label(control_panel, textvariable=self.detection_confidence).grid(
            row=1, column=2, padx=5, pady=5
        )

        # Detect button
        btn_detect = ttk.Button(
            control_panel, text="Run Detection", command=self._run_detection
        )
        ...
        text_frame = ttk.LabelFrame(
            results_frame, text="Detection Results", padding=5
        )
    ...
    def _setup_anomaly_tab(self) -> None:
        ...
        # Control panel
        control_panel = ttk.LabelFrame(
            self.tab_anomaly, text="Anomaly Detection Controls", padding=10
        )
        control_panel.pack(fill=tk.X, padx=10, pady=5)
        ...
        entry_image = ttk.Entry(
            control_panel, textvariable=self.anomaly_image_path, width=50
        )
        entry_image.grid(row=0, column=1, padx=5, pady=5)

        btn_browse = ttk.Button(
            control_panel, text="Browse", command=self._browse_anomaly_image
        )
        btn_browse.grid(row=0, column=2, padx=5, pady=5)
        ...
        orig_frame = ttk.LabelFrame(
            results_frame, text="Original Image", padding=5
        )
        ...
        map_frame = ttk.LabelFrame(
            results_frame, text="Anomaly Map", padding=5
        )
        ...
        # Results text
        text_frame = ttk.LabelFrame(
            self.tab_anomaly, text="Anomaly Results", padding=5
        )
    ...
    def _setup_training_tab(self) -> None:
        ...
        yolo_frame = ttk.LabelFrame(
            self.tab_training, text="YOLOv9 Training", padding=10
        )
        yolo_frame.pack(fill=tk.BOTH, padx=10, pady=5)

        tk.Label(yolo_frame, text="Dataset Path:").grid(
            row=0, column=0, padx=5, pady=5, sticky=tk.W
        )
        self.yolo_dataset_path = tk.StringVar()
        ttk.Entry(
            yolo_frame, textvariable=self.yolo_dataset_path, width=40
        ).grid(row=0, column=1, padx=5, pady=5)
        ttk.Button(
            yolo_frame, text="Browse", command=self._browse_yolo_dataset
        ).grid(row=0, column=2, padx=5, pady=5)

        tk.Label(yolo_frame, text="Epochs:").grid(
        ...
        ttk.Spinbox(
            ...
            from_=1,
            to=500,
            textvariable=self.yolo_epochs,
            width=10,
        ).grid(row=1, column=1, padx=5, pady=5, sticky=tk.W)

        tk.Label(yolo_frame, text="Batch Size:").grid(
            row=2, column=0, padx=5, pady=5, sticky=tk.W
        )
        self.yolo_batch = tk.IntVar(value=16)
        ttk.Spinbox(
            yolo_frame, from_=1, to=64, textvariable=self.yolo_batch, width=10
        ).grid(row=2, column=1, padx=5, pady=5, sticky=tk.W)

        ttk.Button(
            yolo_frame, text="Start Training", command=self._train_yolo
        ).grid(row=3, column=1, padx=5, pady=10)

        # PaDiM Training
        padim_frame = ttk.LabelFrame(
            self.tab_training, text="PaDiM Training", padding=10
        )
        padim_frame.pack(fill=tk.BOTH, padx=10, pady=5)

        tk.Label(padim_frame, text="Training Images:").grid(
            row=0, column=0, padx=5, pady=5, sticky=tk.W
        )
        self.padim_dataset_path = tk.StringVar()
        ttk.Entry(
            padim_frame, textvariable=self.padim_dataset_path, width=40
        ).grid(row=0, column=1, padx=5, pady=5)
        ttk.Button(
            padim_frame, text="Browse", command=self._browse_padim_dataset
        ).grid(row=0, column=2, padx=5, pady=5)

        ttk.Button(
            padim_frame, text="Start Training", command=self._train_padim
        ).grid(row=1, column=1, padx=5, pady=10)

        # Training progress
        progress_frame = ttk.LabelFrame(
            self.tab_training, text="Training Progress", padding=10
        )
    ...
    def _setup_metrics_tab(self) -> None:
        ...
        metrics_frame = ttk.LabelFrame(
            self.tab_metrics, text="System Metrics", padding=10
        )
        ...
        tk.Label(
            det_frame, text="Detection Metrics", font=("Arial", 12, "bold")
        ).pack()
        ...
        tk.Label(
            anom_frame, text="Anomaly Metrics", font=("Arial", 12, "bold")
        ).pack()
        ...
        charts_frame = ttk.LabelFrame(
            self.tab_metrics, text="Performance Charts", padding=10
        )
    ...
    def _browse_padim_dataset(self) -> None:
        ...
        dirname = filedialog.askdirectory(
            title="Select PaDiM Training Images Directory"
        )
        if dirname:
    ...
    def _run_detection(self) -> None:
        ...
        try:
            ...
            result = self.cps_system.yolo_detector.predict(
                image_path, conf_threshold=confidence
            )

            if result.get("success"):
                # Display image with detections
                image = cv2.imread(image_path)
                annotated = self.cps_system.yolo_detector.visualize_detections(
                    image, result["detections"]
                )
                self._display_image(
                    annotated, self.detection_image_label, max_size=(600, 400)
                )

                # Display results text
                self.detection_results_text.delete(1.0, tk.END)
                self.detection_results_text.insert(
                    tk.END, f"Detections: {result['num_detections']}\n"
                )
                self.detection_results_text.insert(
                    tk.END,
                    (
    ...
    def _run_anomaly_detection(self) -> None:
        ...
        try:
            ...
            if result.get("success"):
                ...
                image = cv2.imread(image_path)
                self._display_image(
                    image, self.anomaly_orig_label, max_size=(400, 400)
                )

                # Display anomaly map
                anomaly_map = result["anomaly_map"]
                visualization = (
                    self.cps_system.padim_detector.visualize_anomaly_map(
                        image, anomaly_map
                    )
                )
                self._display_image(
                    visualization, self.anomaly_map_label, max_size=(400, 400)
                )

                # Display results text
                self.anomaly_results_text.delete(1.0, tk.END)
                self.anomaly_results_text.insert(
                    tk.END, f"Anomaly Score: {result['anomaly_score']:.4f}\n"
                )
                self.anomaly_results_text.insert(
                    tk.END, f"Is Anomalous: {result['is_anomalous']}\n"
                )
                self.anomaly_results_text.insert(
                    tk.END, f"Severity: {result['severity_level']}\n"
                )
                self.anomaly_results_text.insert(
                    tk.END,
    ...
    def _display_image(
        self, image: np.ndarray, label: tk.Label, max_size: tuple = (600, 400)
    ) -> None:
            ...
            try:
                ...
                if not data_yaml.exists():
                    messagebox.showwarning(
                        "Warning",
                        "data.yaml not found. Please create it first.",
                    )
                    return
                ...
                if result.get("success"):
                    ...
                    self._log_training(
                        f"Final mAP50: {result['final_metrics']['mAP50']:.4f}"
                    )
                    messagebox.showinfo(
    ...
    def _train_padim(self) -> None:
        ...
        def train_thread():
            ...
            try:
                ...
                result = self.cps_system.padim_detector.train(
                ...
                if result.get("success"):
                    self._log_training("Training completed successfully!")
                    self._log_training(
                        f"Trained on {result['training_images']} images"
                    )
                    messagebox.showinfo(
                        "Success", "Training completed successfully"
                    )
    ...
    def _refresh_metrics(self) -> None:
        ...
        try:
            # Get metrics
            detection_metrics = (
                self.cps_system.metrics_tracker.get_detection_summary()
            )
            anomaly_metrics = (
                self.cps_system.metrics_tracker.get_anomaly_summary()
            )

            # Update detection metrics
            self.detection_metrics_text.delete(1.0, tk.END)
            self.detection_metrics_text.insert(
                tk.END,
                f"Total Detections: {detection_metrics['total_detections']}\n",
            )
            self.detection_metrics_text.insert(
                tk.END,
                (
                    f"Avg Detections/Frame: "
                    f"{detection_metrics['avg_detections_per_frame']:.2f}\n"
                ),
            )
            self.detection_metrics_text.insert(
                tk.END,
                (
                    ...
                    f"{detection_metrics['avg_inference_time_ms']:.2f} ms\n"
                ),
            )
            self.detection_metrics_text.insert(
                tk.END,
                f"Avg Confidence: {detection_metrics['avg_confidence']:.2f}\n",
            )

            # Update anomaly metrics
            self.anomaly_metrics_text.delete(1.0, tk.END)
            self.anomaly_metrics_text.insert(
                tk.END,
                f"Total Predictions: {anomaly_metrics['total_predictions']}\n",
            )
            self.anomaly_metrics_text.insert(
                tk.END, f"Anomaly Count: {anomaly_metrics['anomaly_count']}\n"
            )
            self.anomaly_metrics_text.insert(
                tk.END,
                f"Anomaly Rate: {anomaly_metrics['anomaly_rate']:.2%}\n",
            )
            self.anomaly_metrics_text.insert(
                tk.END,
                f"Avg Score: {anomaly_metrics['avg_anomaly_score']:.4f}\n",
            )
    ...
    def _update_status(self) -> None:
        ...
        try:
            # Update uptime
            system_metrics = (
                self.cps_system.metrics_tracker.get_system_summary()
            )
            self.status_labels["Uptime"].config(
                text=system_metrics["uptime_formatted"]
            )

            # Update model status
            if self.cps_system.yolo_detector.is_trained:
                self.status_labels["YOLOv9 Status"].config(
                    text="Ready", fg="green"
                )

            if self.cps_system.padim_detector.is_trained:
                self.status_labels["PaDiM Status"].config(
                    text="Ready", fg="green"
                )
