...
class AnalyticsEngine:
        ...
        try:
            metric = {
                ...
                "high_confidence_count": sum(
                    1 for d in detection_data.get('detections', [])
    ...
    def get_detection_statistics(self) -> Dict:
        ...
        return {
            "avg_detections": np.mean([m['num_detections'] for m in metrics_list]),
            "avg_inference_time_ms": np.mean([m['inference_time_ms'] for m in metrics_list]),
            "total_processed": len(metrics_list),
            "avg_high_confidence": np.mean([m['high_confidence_count'] for m in metrics_list])
        }
    ...
    def get_anomaly_statistics(self) -> Dict:
        ...
        return {
            "avg_anomaly_score": np.mean([m['anomaly_score'] for m in metrics_list]),
            "anomaly_rate": sum(1 for m in metrics_list if m['is_anomalous']) / len(metrics_list),
            "total_processed": len(metrics_list),
