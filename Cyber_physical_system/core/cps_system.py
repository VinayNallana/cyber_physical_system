...
class CyberPhysicalSystem:
    ...
    def _initialize_physical_layer(self) -> None:
        ...
        self.environmental_sensors = {
            "temp_1": EnvironmentalSensor("temp_1", "temperature", "°C", (20.0, 80.0)),
            "pressure_1": EnvironmentalSensor("pressure_1", "pressure", "PSI", (10.0, 100.0)),
            "vibration_1": EnvironmentalSensor("vibration_1", "vibration", "mm/s", (0.0, 50.0))
        }
        ...
        logger.info(f"Physical layer initialized: {len(self.camera_sensors)} cameras, "
                   f"{len(self.environmental_sensors)} sensors, {len(self.actuators)} actuators")
    ...
    def _setup_message_subscriptions(self) -> None:
        ...
        self.message_broker.subscribe("detection/yolo", self._handle_detection_message)
        self.message_broker.subscribe("detection/padim", self._handle_anomaly_message)
        
        # Subscribe analytics to all events
        self.message_broker.subscribe("detection/yolo", self._handle_analytics_update)
        self.message_broker.subscribe("detection/padim", self._handle_analytics_update)
    ...
    def _handle_detection_message(self, message: Dict) -> None:
        ...
        try:
            ...
            if payload.get("success"):
                ...
                decision = self.decision_engine.process_detection_results(payload)
    ...
    def _handle_anomaly_message(self, message: Dict) -> None:
        ...
        try:
            ...
            if payload.get("success"):
                # Process in decision engine
                decision = self.decision_engine.process_anomaly_results(payload)
    ...
    def _execute_control_action(self, decision: Dict) -> None:
        ...
        try:
            ...
            if action == "emergency_stop":
                ...
                self.actuators["motor_1"].execute_command("reduce_speed", {"speed": 50})
                self.actuators["alarm_1"].execute_command("activate")
            
            # Publish control message
            self.message_broker.publish("control/actuators", {
                "action": action,
                "priority": priority,
    ...
    def process_image_yolo(self, image_path: str, conf_threshold: float = 0.5) -> Dict:
        """
        ...
        try:
            # Run detection
            result = self.yolo_detector.predict(image_path, conf_threshold=conf_threshold)
    ...
    def train_yolo(self, data_yaml: str, epochs: int = 100, 
        ...
        Returns:
            Training results
        """
        return self.yolo_detector.train(data_yaml, epochs, batch_size=batch_size, **kwargs)
    ...
    def load_yolo_model(self, model_path: str, class_names: Optional[List[str]] = None) -> bool:
    ...
    def get_system_status(self) -> Dict:
        ...
        return {
            ...
            "timestamp": datetime.now(),
            "physical_layer": {
                "cameras": {k: v.is_active for k, v in self.camera_sensors.items()},
                "sensors": {k: v.is_active for k, v in self.environmental_sensors.items()},
                "actuators": {k: v.is_active for k, v in self.actuators.items()}
            },
            "models": {
