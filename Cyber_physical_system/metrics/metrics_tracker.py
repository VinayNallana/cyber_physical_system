...
class MetricsTracker:
    ...
    def update_detection_metrics(self, detection_result: Dict) -> None:
        ...
        try:
            ...
            self.detection_metrics["detections_per_frame"].append(len(detections))
            ...
            for detection in detections:
                ...
                if class_name in self.detection_metrics["class_distribution"]:
                    self.detection_metrics["class_distribution"][class_name] += 1
                else:
                    self.detection_metrics["class_distribution"][class_name] = 1
    ...
    def get_detection_summary(self) -> Dict:
        ...
        return {
            ...
            "avg_detections_per_frame": np.mean(list(self.detection_metrics["detections_per_frame"])),
            "avg_inference_time_ms": np.mean(list(self.detection_metrics["inference_times"])),
            "max_inference_time_ms": np.max(list(self.detection_metrics["inference_times"])),
            "avg_confidence": np.mean(list(self.detection_metrics["confidence_scores"])) if self.detection_metrics["confidence_scores"] else 0,
    ...
    def get_anomaly_summary(self) -> Dict:
        ...
        return {
            ...
            "anomaly_rate": self.anomaly_metrics["anomaly_count"] / self.anomaly_metrics["total_predictions"],
            "avg_anomaly_score": np.mean(list(self.anomaly_metrics["anomaly_scores"])),
            "max_anomaly_score": np.max(list(self.anomaly_metrics["anomaly_scores"])),
            "avg_inference_time_ms": np.mean(list(self.anomaly_metrics["inference_times"])),
    ...
    def get_performance_summary(self) -> Dict:
        ...
        return {
            "avg_cpu_usage": np.mean(list(self.performance_metrics["cpu_usage"])),
            "max_cpu_usage": np.max(list(self.performance_metrics["cpu_usage"])) if self.performance_metrics["cpu_usage"] else 0,
            "avg_memory_usage": np.mean(list(self.performance_metrics["memory_usage"])),
            "max_memory_usage": np.max(list(self.performance_metrics["memory_usage"])) if self.performance_metrics["memory_usage"] else 0,
            "avg_fps": np.mean(list(self.performance_metrics["fps"])),
            "min_fps": np.min(list(self.performance_metrics["fps"])) if self.performance_metrics["fps"] else 0
        }
