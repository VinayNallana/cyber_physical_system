...
class DatasetManager:
    ...
    def _reorganize_yolo_files(self, dataset_path: Path) -> Dict:
        ...
        try:
            ...
            for img in train_images:
                ...
                if label.exists():
                    shutil.copy(label, dataset_path / "labels" / "train" / label.name)
            
            for img in val_images:
                shutil.copy(img, dataset_path / "images" / "val" / img.name)
                label = img.with_suffix('.txt')
                if label.exists():
                    shutil.copy(label, dataset_path / "labels" / "val" / label.name)
    ...
    def organize_padim_dataset(self, dataset_path: str) -> Dict:
        ...
        try:
            ...
            if train_dir.exists() and test_dir.exists():
                ...
                train_good = len(list((train_dir / "good").glob("*"))) if (train_dir / "good").exists() else 0
                
                test_categories = {}
                if test_dir.exists():
                    for category_dir in test_dir.iterdir():
                        if category_dir.is_dir():
                            test_categories[category_dir.name] = len(list(category_dir.glob("*")))
    ...
    def create_data_yaml(self, dataset_path: str, class_names: List[str],
                         nc: Optional[int] = None) -> str:
        """
